...
A binary number with no fractional part may be converted to the corresponding decimal integer by adding powers of 2. If the $j\text{th}$ bit is 1, then the sum should include the term $2^{j}$.
...
$$
x=\left(-1\right)^{s}\cdot\left(i_{1}2^{0}+i_{2}2^{1}+i_{3}2^{2}+\cdots+i_{31}2^{30}\right)
  =\left(-1\right)^{s}\sum_{k=1}^{31}i_{k}2^{k-1},
$$
...
$$
x=\left(i_{1}2^{0}+i_{2}2^{1}+i_{3}2^{2}+\cdots+i_{31}2^{30}+i_{32}2^{31}\right)-2^{31}
  =\sum_{k=1}^{32}i_{k}2^{k-1}-2^{31}.
$$

To represent 1, we will have $i_{1}=1$, $i_{32}=1$, and $i_{k}=0$ for $k\in\left\{ 2,3,\ldots,31\right\}$. The next consecutive integer is 2, which we represent with $i_{2}=1$, $i_{32}=1$, and $i_{k}=0$ for $k\in\left\{ 1\right\} \cup\left\{ 3,4,\ldots,31\right\}$. We represent 3 as $i_{1}=1$, $i_{2}=1$, $i_{32}=1$, and $i_{k}=0$ for $k\in\left\{ 3,4,\ldots,31\right\}$. We proceed in this fashion until $i_{k}=1$ for $k\in\left\{ 1,2,\ldots,32\right\}$, i.e., 
...
We first consider the exponent. More precisely, the $e_{k}$ bits represent the positive binary integer that is the sum of the exponent and the _bias_ $2^{10}-1=1023$ (for exponents between -1022 and 1023). Letting $p$ be the decimal exponent, we will consider some examples.
...
-----:| --------:| -------------------------:| ---------------------------------:|
 ...
 $17$ | $1033$   | $100\,0001\,0001$         | $\mathtt{100\,0001\,0001}$        |

Thus, given some $\boxed{e_{1}e_{2}\ldots e_{11}}$, we can recover the exponent $p$ by subtracting the bias.

We now consider the mantissa. @sauer2011 describes the _normalized_ floating-point representation of a binary number as being "left-justified, meaning that the leftmost 1 is shifted just to the left of the radix point" (generalization of a decimal point). We can change the exponent to compensate for shifting the radix, i.e., the radix "floats." Now, the $1$ in $1.\boxed{b_{1}b_{2}\ldots b_{52}}$ is assumed, _not stored_. More formally, we have
...
so that $s=0$, $b_{j}=1$ for $j=3$, and $e_{k}=1$ for $k\in\left\{1,10\right\}$. As a check, note that
...
so that $s=0$, $b_{j}=1$ for $j\in\left\{4,5,7\right\}$, and $e_{k}=1$ for $k\in\left\{1,10,11\right\}$. As a check, note that
...
Now, the exponent value $2047_{10}=\left(111\,1111\,1111\right)_{2}$ is reserved to represent infinity if every $b_{j}$ is zero, i.e., if the mantissa bits are all zero, and to represent `NaN` (Not a Number) otherwise. The exponent value 0 is used to represent _subnormal_ floating point numbers, or those numbers where the left-most bit is not assumed to be 1. Thus, the smallest non-reserved value the exponent bits can take is $\mathtt{000\,0000\,0001}$, which corresponds to an exponent of $2^{0}-1023=-1022$. The largest non-reserved value the exponent bits can take is $\mathtt{111\,1111\,1110}$, which corresponds to an exponent of $\sum_{k=1}^{10}2^{k}-1023=1023$.

Thus, the range of the exponent is $\left(-2^{10}+2,2^{10}-1\right)$, so that the largest number that can be represented using the double precision floating-point encoding is roughly $2^{2^{10}-1}=2^{1023}$. To be precise, we must also consider the mantissa. If all the mantissa bits are 1, then 
...
Now we pretend that we do not know $\mathbf{x}$ and will solve for it. Comparing $\mathbf{x}$ to the numerical solution $\hat{\mathbf{x}}$, we see that some elements are close, but in most cases the solution is highly inaccurate.
